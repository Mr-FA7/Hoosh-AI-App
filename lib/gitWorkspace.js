...
function pull(projectRoot) {
  return runGit('pull --ff-only', projectRoot || process.cwd());
}

function push(projectRoot) {
  return runGit('push', projectRoot || process.cwd());
}

module.exports = { getStatus, getDiff, stageFiles, commit, getLog, pull, push, runGit };
