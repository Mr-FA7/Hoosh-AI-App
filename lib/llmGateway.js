...
class LlmGateway {
  ...
  resolveProvider(modelName, role) {
    ...
    if (m.includes('/')) {
      ...
      if (this.config.providers[prov]) {
        return { providerId: prov, model: rest.join('/') };
      }
    }
    // Ollama model names look like "name:tag" (contain : but not /) — always route to ollama
    // This prevents them from falling through to cloud providers via defaultProvider
    if (m.includes(':') && !m.includes('/')) {
      return { providerId: 'ollama', model: m };
    }
  ...
  async fetchOpenAiCompatibleModels(providerId, prov) {
    const base = String(prov.baseUrl || '').replace(/\/$/, '');
    if (!base) return [];
    const headers = {};
    if (prov.apiKey) headers.Authorization = `Bearer ${prov.apiKey}`;
    try {
      const r = await axios.get(`${base}/models`, { headers, timeout: 8000 });
      const rows = Array.isArray(r.data?.data) ? r.data.data : [];
      const labelPrefix = prov.label || (providerId === 'lmstudio' ? 'LM Studio' : providerId);
      return rows
        .map((m) => {
  ...
  async listModels() {
    ...
    try {
      ...
      for (const m of r.data?.models || []) {
        push({ name: m.name, provider: 'ollama', label: m.name, source: 'local' });
      }
    } catch { /* ignore */ }

    for (const [id, prov] of Object.entries(this.config.providers || {})) {
      if (!prov || prov.type === 'ollama') continue;
      const isLocalDiscoverable = id === 'lmstudio' || id === 'llamacpp';
      if (prov.type === 'openai') {
        // Local discoverable providers (LM Studio, llama.cpp): only show if actually running
        if (isLocalDiscoverable) {
          const fetched = await this.fetchOpenAiCompatibleModels(id, prov);
          fetched.forEach(push);  // only adds models if service is actually running
          continue;
        }
        // Cloud/user-configured providers: only show if explicitly enabled with an API key
        if (!prov.enabled || !prov.apiKey) continue;
        const fetched = await this.fetchOpenAiCompatibleModels(id, prov);
        if (fetched.length) {
          fetched.forEach(push);
        } else if (prov.model) {
          // Show placeholder only if enabled with a key
          push({
            name: `${id}/${prov.model}`,
            provider: id,
            label: `${prov.label || id}: ${prov.model}`,
            source: 'cloud'
          });
        }
        continue;
      }
      if (prov.type === 'anthropic' && prov.enabled && prov.apiKey) {
        push({
          name: `${id}/${prov.model || 'default'}`,
          provider: id,
          label: `${prov.label || id}: ${prov.model || 'default'}`,
          source: 'cloud'
        });
      }
    }
  ...
  async chatStream(payload, options = {}) {
    ...
    if (prov.type === 'ollama' || providerId === 'ollama') {
      ...
      // Disable thinking mode for qwen3/deepseek-r1 thinking models
      const _isThinking = /qwen3|deepseek-r1|qwq/i.test(String(resolvedModel || ''));
      const _ollamaPayload = { ...payload, model: resolvedModel };
      if (_isThinking && _ollamaPayload.think === undefined) _ollamaPayload.think = false;
      return axios.post(`${base}/api/chat`, _ollamaPayload, {
        responseType: 'stream',
