...
class LlmGateway {
  ...
  async listModels() {
    ...
    for (const [id, prov] of Object.entries(this.config.providers || {})) {
      ...
      const autoDiscoverLocal = id === 'lmstudio' || id === 'llamacpp';
      if (prov.type === 'openai') {
        if (!prov.enabled && !autoDiscoverLocal) continue;
        const fetched = await this.fetchOpenAiCompatibleModels(id, prov);
        if (fetched.length) {
          fetched.forEach(push);
          continue;
        }
        if (prov.enabled && prov.model) {
          push({
            name: `${id}/${prov.model}`,
            provider: id,
      ...
      if (prov.type === 'anthropic' && prov.enabled) {
        push({
