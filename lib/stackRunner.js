...
async function composeExec(projectRoot, options = {}) {
  const file = resolveComposeFile(projectRoot, options.composeFile);
  if (!file) return { ok: false, error: 'No compose file found' };
  const service = String(options.service || '').trim();
  const command = String(options.command || '').trim();
  if (!service || !command) return { ok: false, error: 'service and command required' };
  const args = ['exec', '-T', service, '/bin/sh', '-lc', command];
  const r = await runCompose(projectRoot, args, { file, timeoutMs: options.timeoutMs || 120000 });
  return { ok: r.ok, stdout: r.stdout, stderr: r.stderr, output: (r.stdout || '') + (r.stderr || '') };
}

async function composePull(projectRoot, options = {}) {
  const file = resolveComposeFile(projectRoot, options.composeFile);
  if (!file) return { ok: false, error: 'No compose file found' };
  const args = ['pull'];
  if (options.services?.length) args.push(...options.services);
...
async function composeRestart(projectRoot, options = {}) {
  const file = resolveComposeFile(projectRoot, options.composeFile);
  if (!file) return { ok: false, error: 'No compose file found' };
  const args = ['restart'];
  if (options.services?.length) args.push(...options.services);
...
async function composeBuild(projectRoot, options = {}) {
  const file = resolveComposeFile(projectRoot, options.composeFile);
  if (!file) return { ok: false, error: 'No compose file found' };
  const args = ['build'];
  if (options.noCache) args.push('--no-cache');
  if (options.services?.length) args.push(...options.services);
  const r = await runCompose(projectRoot, args, { file, timeoutMs: options.timeoutMs || 600000 });
  return { ok: r.ok, stdout: r.stdout, stderr: r.stderr };
}

async function composeConfig(projectRoot, composeFile) {
  const file = resolveComposeFile(projectRoot, composeFile);
  if (!file) return { ok: false, error: 'No compose file found' };
  const r = await runCompose(projectRoot, ['config'], { file, timeoutMs: 30000 });
  return { ok: r.ok, composeFile: file, config: r.stdout, stderr: r.stderr };
}
...
class StackRunner {
  ...
  exec(options) { return composeExec(this.projectRoot, options); }
  pull(options) { return composePull(this.projectRoot, options); }
  restart(options) { return composeRestart(this.projectRoot, options); }
  build(options) { return composeBuild(this.projectRoot, options); }
  config(options) { return composeConfig(this.projectRoot, options?.composeFile); }
