...
class SubagentRunner {
  ...
  async runParallel(goal, contextStr, options = {}) {
    const { count = 3, onEvent, timeoutMs = 90000 } = options;
    ...
    const tasks = angles.map(async (angle) => {
      ...
      try {
        // Research is best-effort context, never a reason to stall the whole
        // mission: one hung provider call used to block Promise.all forever.
        let timer;
        const result = await Promise.race([
          this.kernel.runAgentTask('coder', prompt, {
            lang: 'en',
            priority: 'low',
            tier: 'light'   // parallel research reads run on the fast/light model
          }),
          new Promise((resolve) => {
            timer = setTimeout(() => resolve(`(research timed out after ${Math.round(timeoutMs / 1000)}s)`), timeoutMs);
          })
