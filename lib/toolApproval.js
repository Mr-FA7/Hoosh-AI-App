...
function categorizeTool(toolName) {
  ...
  if (['readfile', 'readdir', 'listdir', 'websearch', 'search', 'deepresearch', 'research', 'fetchurl', 'fetch', 'changedirectory', 'recall', 'remember', 'memory', 'glob', 'grep'].includes(t)) return 'read';
  if (['writefile', 'patchfile', 'deletefile', 'createdir', 'buildcompose', 'buildcomposeartifacts'].includes(t)) return 'write';
  if (/^stack|^container|^image|^flow|^automation|^kube|^network|^volume|^runtime/.test(t)) return 'terminal';
